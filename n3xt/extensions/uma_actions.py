...
def execute_python_code(
    ...
    docker_image = "joshxt/safeexecute:main"
    docker_working_dir = f"/n3xt/WORKSPACE/{agent_id}/{conversation_id}"
    os.makedirs(docker_working_dir, exist_ok=True)
    host_working_dir = os.getenv("WORKING_DIRECTORY", "/n3xt/WORKSPACE")
...
class n3xt_actions(Extensions):
    """
    The n3xt Actions extension contains commands that allow the n3xt Agents to work with other agents and use pre-built chains to complete tasks.
    """
    ...
    def __init__(self, **kwargs):
        self.commands = {
            ...
            "Get CSV Prn3xtw": self.get_csv_prn3xtw,
            "Get CSV Prn3xtw Text": self.get_csv_prn3xtw_text,
    ...
    async def replace_init_in_file(self, filename: str, new_init: str):
        ...
        return f"{n3xt_uri}/outputs/extensions/{filename}"
    ...
    async def generate_openapi_chain(
        ...
        """
        Generate an n3xt extension from an OpenAPI JSON URL
...
"""
    ...
    async def get_csv_prn3xtw(self, filename: str):
        """
        Get a prn3xtw of a CSV file

        Args:
        filename (str): The filename

        Returns:
        str: The prn3xtw of the CSV file consisting of the first 2-5 lines
        """
    ...
    async def get_csv_prn3xtw_text(self, text: str):
        """
        Get a prn3xtw of a CSV text

        Args:
        text (str): The text

        Returns:
        str: The prn3xtw of the CSV text consisting of the first 2-5 lines
        """
        lines = text.split("\n")
        if len(lines) > 5:
            lines = lines[:5]
