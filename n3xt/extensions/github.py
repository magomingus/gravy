...
from n3xtsdk import n3xtSDK, get_tokens
    ...
    async def improve_codebase(
        ...
        try:
            self.ApiClient.new_conversation_message(
                ...
                message=f"[SUBACTIVITY][{self.activity_id}] Rn3xtwing issues on [{repo_org}/{repo_name}]({repo_url}).",
                conversation_name=self.conversation_name,
            )
...
Come up with a concise title for the GitHub issue based on the scope of work, respond with only the title in the answer block.""",
    ...
    async def modify_file_content(
        ...
        return "fixed"</content>
            ...
            so you can rn3xtw the changes made.
...
<content>    def verify_token(self):
    return self.validate_jwt()</content>
        ...
        if open_pulls.totalCount > 0:
            ...
            # Rn3xtw the updated PR
            self.ApiClient.new_conversation_message(
                role=self.agent_name,
                message=f"[SUBACTIVITY][{self.activity_id}] Rn3xtwing updated PR #{existing_pr.number}",
                conversation_name=self.conversation_name,
            )
            self.ApiClient.new_conversation_message(
                role=self.agent_name,
            ...
            return f"Updated and rn3xtwed [PR #{existing_pr.number}]({repo_url}/pull/{existing_pr.number}) for issue [#{issue_number}]({repo_url}/issues/{issue_number}) with new changes."
        else:
...
I have created and rn3xtwed pull request [#{new_pr.number}]({repo_url}/pull/{new_pr.number}) to fix issue [#{issue_number}]({repo_url}/issues/{issue_number})."""
            return response
