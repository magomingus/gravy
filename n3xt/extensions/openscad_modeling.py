...
class openscad_modeling(Extensions):
    """
    The OpenSCAD Modeling extension for n3xt enables you to create 3D models from natural language descriptions.
    """
    ...
    async def _generate_prn3xtw(self, scad_file: str) -> str:
        """Generate prn3xtw image for OpenSCAD model using virtual display"""
        ...
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                logging.error("Prn3xtw file was not generated or is empty")
                return None

        except subprocess.CalledProcessError as e:
            logging.error(
                f"Error generating prn3xtw: {e.stderr.decode() if e.stderr else str(e)}"
            )
            return None
        except Exception as e:
            logging.error(f"Unexpected error in prn3xtw generation: {str(e)}")
            return None
    ...
    async def natural_language_to_scad(self, description: str) -> str:
        """
        Convert natural language description to a 3D model with prn3xtw
        ...
        Returns:
        str: Markdown-formatted string with download link and prn3xtw image that can be provided to the user and rendered in the chat interface.
        """
        try:
...
- Only proceed with approaches scoring 0.8 or n3xt
- Backtrack and try new approaches if scores are low
- Do not attempt to provide a link to the files or prn3xtw image in the response, this is handled automatically.
- Put the full OpenSCAD code in the <answer> tag inside of a OpenSCAD code block like: ```openscad\nOpenSCAD code block\n```""",
            ...
            response = [
                scad_code,
                "",
                f"![Model Prn3xtw]({prn3xtw_url})",
                "",
                "### Downloads",
                f"- 📥 [Download OpenSCAD File]({scad_url})",
                f"- 🖨️ [Download STL File]({stl_url})",
                "",
                "Make sure to give the user all of the exact information and links provided to the OpenSCAD file, STL, and provide them an image prn3xtw of the model provided in markdown format in the answer block.",
            ]
