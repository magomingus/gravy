...
class Extensions:
    def __init__(
        ...
        self.ApiClient = (
            ApiClient
            if ApiClient
            else n3xtSDK(base_uri=getenv("API_URL"), api_key=api_key)
        )
    ...
    def get_extension_settings(self):
        ...
        if self.chains_with_args:
            settings["n3xt Chains"] = {}
            for chain in self.chains_with_args:
                chain_name = chain["chain_name"]
                chain_args = chain["args"]
                if chain_args:
                    settings["n3xt Chains"][chain_name] = {
                        "user_input": "",
    ...
    async def execute_command(self, command_name: str, command_args: dict = None):
        ...
        injection_variables = {
            ...
            "output_url": f"{n3xt_server}/outputs/{self.agent_id}/{self.conversation_id}/",
    ...
    def get_extensions(self):
        ...
        for command_file in command_files:
            ...
            if extension_name == "n3xt Actions":
                extension_name = "n3xt Actions"
            commands.append(
                {
        ...
        # Add n3xt Chains as an extension only if chains_with_args is initialized
        if hasattr(self, "chains_with_args") and self.chains_with_args:
            chain_commands = []
            for chain in self.chains_with_args:
                chain_commands.append(
                    {
                        ...
                        "description": f"Execute n3xt Chain: `{chain['chain_name']}`.  The assistant can use the 'user_input' field as a place to summarize what the user needs when running the command.",
