...
- n3xt_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- n3xt_URI: URL to the n3xt server
...
def is_n3xt_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("n3xt_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    n3xt_API_KEY = getenv("n3xt_API_KEY")
    authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
    if n3xt_API_KEY:
        if authorization == n3xt_API_KEY:
        ...
        logging.error(
            "n3xt API Key is missing. Please set the n3xt_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("n3xt_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                n3xt = n3xtSDK(base_uri=getenv("n3xt_URI"))
                n3xt.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                n3xt.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                n3xt.add_agent(
                    agent_name="n3xt",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        n3xt = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> n3xtSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> n3xtSDK:
        if not company_id:
            company_id = self.company_id
        ...
        n3xt.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return n3xt
