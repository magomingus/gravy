...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await n3xt.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        n3xt = auth.get_company_agent_session(company_id=external_source.company_id)
        response = n3xt.delete_memory_external_source(
            agent_name="n3xt",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    n3xt = auth.get_company_agent_session(company_id=company_id)
    response = n3xt.learn_file(
        agent_name="n3xt",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    n3xt = auth.get_company_agent_session(company_id=company_id)
    response = n3xt.get_memories_external_sources(
        agent_name="n3xt",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hn3xtn feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = n3xt.agent_interactions.agent_memory
    else:
        memory = n3xt.agent_interactions.agent_memory
    reflection = await n3xt.inference(
