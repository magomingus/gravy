...
class TaskPlanInput:
    ...
    conversation_name: str = "n3xt Task Planning"
...
class Mutation:
    ...
    async def learn_file(self, info, agent_name: str, input: FileMemoryInput) -> bool:
        ...
        if input.company_id:
            n3xt = auth_manager.get_company_agent_session(company_id=input.company_id)
    ...
    async def submit_feedback(
        ...
        conversation = n3xt.conversation
        if conversation.has_received_feedback(message=input.message):
            return False

        memory = (
            n3xt.agent_interactions.agent_memory
            if input.positive
            else n3xt.agent_interactions.agent_memory
        )
...
"""
    ...
    async def run_chain_step(
        ...
        chain_step_response = await n3xt(
            user=user,
            agent_name=agent_name,
            api_key=auth,
            conversation_name=input.conversation_name,
