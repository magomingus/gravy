        ...
        try:
            ...
            if company_id:
                ...
                company_memories = cs.get_agent_memories(
                    agent_name="n3xt", user_input=user_input
                )
                if company_memories:
    ...
    async def run(
        ...
        **kwargs,
        ...
        if self.outputs in formatted_prompt:
            # Anonymize n3xt server URL to LLM
        ...
        # Deanonymize n3xt server URL to send back to the user
                    ...
                    try:
                        ...
                        if not str(tts_response).startswith("http"):
                            ...
                            with open(audio_path, "wb") as f:
                                f.write(audio_data)
                            tts_response = f'<audio controls><source src="{n3xt_URI}/outputs/{self.agent.agent_id}/{file_name}" type="audio/wav"></audio>'
