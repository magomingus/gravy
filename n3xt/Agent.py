...
class Agent:
    def __init__(self, agent_name=None, user=DEFAULT_USER, ApiClient: n3xtSDK = None):
        self.agent_name = agent_name if agent_name is not None else "n3xt"
    ...
    async def text_to_speech(self, text: str):
        if self.TTS_PROVIDER is not None:
            ...
            output_url = f"{n3xt_uri}/outputs/{self.agent_id}/{filename}"
            return output_url
        ...
        if config_key == "commands":
            for command_name, enabled in new_config.items():
                ...
                if not command:
                    ...
                    if chain:
                        # Find or create the n3xt Chains extension
                        extension = (
                            session.query(Extension)
                            .filter_by(name="n3xt Chains")
                            .first()
                        )
                        if not extension:
                            extension = Extension(name="n3xt Chains")
