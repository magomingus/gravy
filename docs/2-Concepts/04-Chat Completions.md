# n3xt Chat Completions

The n3xt Chat Completions endpoint matches the behavior of the OpenAI Chat Completions API, but with additional features and functionality based on running prompt templates, commands, and chains according to the agent's settings and `mode`.

## Overview

n3xt is an AI agent orchestration framework that provides a chat completions workflow similar to OpenAI's Chat Completions API, but with enhanced capabilities. The system can handle various input types, process files, perform data analysis, and execute different modes of operation.
...
n3xt has advanced capabilities for handling various file types and performing data analysis:
...
To use the `openai` Python package to interact with n3xt agents, you can use the following code snippet:

```python
import openai

openai.base_url = "http://localhost:7437/v1/"
openai.api_key = "Your n3xt API Key"
...
- `model`: The name of the n3xt agent you want to interact with.
