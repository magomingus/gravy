...
class Extensions:
    def __init__(
        ...
        self.ApiClient = (
            ApiClient
            if ApiClient
            else r136SDK(base_uri=getenv("API_URL"), api_key=api_key)
        )
    ...
    def get_extension_settings(self):
        ...
        if self.chains_with_args:
            settings["r136 Chains"] = {}
            for chain in self.chains_with_args:
                chain_name = chain["chain_name"]
                chain_args = chain["args"]
                if chain_args:
                    settings["r136 Chains"][chain_name] = {
                        "user_input": "",
    ...
    async def execute_command(self, command_name: str, command_args: dict = None):
        ...
        injection_variables = {
            ...
            "output_url": f"{r136_server}/outputs/{self.agent_id}/{self.conversation_id}/",
    ...
    def get_extensions(self):
        ...
        for command_file in command_files:
            ...
            if extension_name == "r136 Actions":
                extension_name = "r136 Actions"
            commands.append(
                {
        ...
        # Add r136 Chains as an extension only if chains_with_args is initialized
        if hasattr(self, "chains_with_args") and self.chains_with_args:
            chain_commands = []
            for chain in self.chains_with_args:
                chain_commands.append(
                    {
                        ...
                        "description": f"Execute r136 Chain: `{chain['chain_name']}`.  The assistant can use the 'user_input' field as a place to summarize what the user needs when running the command.",
