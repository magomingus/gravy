...
- r136_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- r136_URI: URL to the r136 server
...
def is_r136_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("r136_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    r136_API_KEY = getenv("r136_API_KEY")
    authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
    if r136_API_KEY:
        if authorization == r136_API_KEY:
        ...
        logging.error(
            "r136 API Key is missing. Please set the r136_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("r136_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                r136 = r136SDK(base_uri=getenv("r136_URI"))
                r136.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                r136.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                r136.add_agent(
                    agent_name="r136",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        r136 = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> r136SDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> r136SDK:
        if not company_id:
            company_id = self.company_id
        ...
        r136.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return r136
