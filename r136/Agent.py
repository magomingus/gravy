...
class Agent:
    def __init__(self, agent_name=None, user=DEFAULT_USER, ApiClient: r136SDK = None):
        self.agent_name = agent_name if agent_name is not None else "r136"
    ...
    async def text_to_speech(self, text: str):
        if self.TTS_PROVIDER is not None:
            ...
            output_url = f"{r136_uri}/outputs/{self.agent_id}/{filename}"
            return output_url
        ...
        if config_key == "commands":
            for command_name, enabled in new_config.items():
                ...
                if not command:
                    ...
                    if chain:
                        # Find or create the r136 Chains extension
                        extension = (
                            session.query(Extension)
                            .filter_by(name="r136 Chains")
                            .first()
                        )
                        if not extension:
                            extension = Extension(name="r136 Chains")
