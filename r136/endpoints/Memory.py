...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await r136.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        r136 = auth.get_company_agent_session(company_id=external_source.company_id)
        response = r136.delete_memory_external_source(
            agent_name="r136",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    r136 = auth.get_company_agent_session(company_id=company_id)
    response = r136.learn_file(
        agent_name="r136",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    r136 = auth.get_company_agent_session(company_id=company_id)
    response = r136.get_memories_external_sources(
        agent_name="r136",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hr136n feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = r136.agent_interactions.agent_memory
    else:
        memory = r136.agent_interactions.agent_memory
    reflection = await r136.inference(
