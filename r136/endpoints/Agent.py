...
from Globals import getenv, get_default_agent, get_r136_training_urls
...
async def getagents(user=Depends(verify_api_key), authorization: str = Header(None)):
    ...
    if create_agent:
        ...
        if agent_name not in agent_list:
            ...
            create_r136_agent = str(getenv("CREATE_r136_AGENT")).lower() == "true"
            training_urls = (
                get_r136_training_urls()
                if create_r136_agent and agent_name == "r136"
...
async def text_to_speech(
    ...
    if not str(tts_response).startswith("http"):
        ...
        with open(audio_path, "wb") as f:
            f.write(audio_data)
        tts_response = f"{r136_URI}/outputs/{agent.agent_id}/{file_name}"
...
async def think(
    ...
    think_deep = f"**The assistant should think deeply on the user's input and available context, reflect while being critical of the assistant's thoughts to ensure the most well reasoned response to the user. The assistant has 100 available thinking and reflection tokens to think through the response and is encouraged to use them liberally anywhere the assistant finds a r136 reward could be achieved on responses. During reflection, the assistant should act as a critical judge that expects only the best, most well reasoned thoughts before answering the user, and should provide guidance for the next thoughts to be guided by for improved reasoning.**"
