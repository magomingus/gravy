...
class openscad_modeling(Extensions):
    """
    The OpenSCAD Modeling extension for r136 enables you to create 3D models from natural language descriptions.
    """
    ...
    async def _generate_prr136w(self, scad_file: str) -> str:
        """Generate prr136w image for OpenSCAD model using virtual display"""
        ...
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                logging.error("Prr136w file was not generated or is empty")
                return None

        except subprocess.CalledProcessError as e:
            logging.error(
                f"Error generating prr136w: {e.stderr.decode() if e.stderr else str(e)}"
            )
            return None
        except Exception as e:
            logging.error(f"Unexpected error in prr136w generation: {str(e)}")
            return None
    ...
    async def natural_language_to_scad(self, description: str) -> str:
        """
        Convert natural language description to a 3D model with prr136w
        ...
        Returns:
        str: Markdown-formatted string with download link and prr136w image that can be provided to the user and rendered in the chat interface.
        """
        try:
...
- Only proceed with approaches scoring 0.8 or r136
- Backtrack and try new approaches if scores are low
- Do not attempt to provide a link to the files or prr136w image in the response, this is handled automatically.
- Put the full OpenSCAD code in the <answer> tag inside of a OpenSCAD code block like: ```openscad\nOpenSCAD code block\n```""",
            ...
            response = [
                scad_code,
                "",
                f"![Model Prr136w]({prr136w_url})",
                "",
                "### Downloads",
                f"- 📥 [Download OpenSCAD File]({scad_url})",
                f"- 🖨️ [Download STL File]({stl_url})",
                "",
                "Make sure to give the user all of the exact information and links provided to the OpenSCAD file, STL, and provide them an image prr136w of the model provided in markdown format in the answer block.",
            ]
