...
from r136sdk import r136SDK, get_tokens
    ...
    async def improve_codebase(
        ...
        try:
            self.ApiClient.new_conversation_message(
                ...
                message=f"[SUBACTIVITY][{self.activity_id}] Rr136wing issues on [{repo_org}/{repo_name}]({repo_url}).",
                conversation_name=self.conversation_name,
            )
...
Come up with a concise title for the GitHub issue based on the scope of work, respond with only the title in the answer block.""",
    ...
    async def modify_file_content(
        ...
        return "fixed"</content>
            ...
            so you can rr136w the changes made.
...
<content>    def verify_token(self):
    return self.validate_jwt()</content>
        ...
        if open_pulls.totalCount > 0:
            ...
            # Rr136w the updated PR
            self.ApiClient.new_conversation_message(
                role=self.agent_name,
                message=f"[SUBACTIVITY][{self.activity_id}] Rr136wing updated PR #{existing_pr.number}",
                conversation_name=self.conversation_name,
            )
            self.ApiClient.new_conversation_message(
                role=self.agent_name,
            ...
            return f"Updated and rr136wed [PR #{existing_pr.number}]({repo_url}/pull/{existing_pr.number}) for issue [#{issue_number}]({repo_url}/issues/{issue_number}) with new changes."
        else:
...
I have created and rr136wed pull request [#{new_pr.number}]({repo_url}/pull/{new_pr.number}) to fix issue [#{issue_number}]({repo_url}/issues/{issue_number})."""
            return response
